...
bool SimpleGraph::edgeExists(uint32_t from, uint32_t to, uint32_t edgeLabel) {
    auto it = std::find(SO[from].begin(), SO[from].end(), to);
    return (it != SO[from].end());
...
//            addEdge(subject, object, predicate);
            ...
            PSO[predicate].push_back(std::pair<uint32_t, uint32_t>(subject, object));
            POS[predicate].push_back(std::pair<uint32_t, uint32_t>(object, subject));
        }
    }
